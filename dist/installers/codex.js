...
function vendoredCodexPluginDir() {
...
export function installCodex(env = process.env) {
    const vendorDir = vendoredCodexPluginDir();
    if (!fs.existsSync(path.join(vendorDir, 'scripts', 'install-codex-hooks.cjs'))) {
        throw new Error(`vendored Codex plugin scripts not found at ${vendorDir} — this dmemo build is broken ` +
