...
import { rpcUrlFor, chainIdFor, chainIdHexFor, chainNameFor, checkBalance, faucetInstructions, CURRENCY_SYMBOL, FAUCET_URL, } from './network.js';
...
import { derivationMessage, verifyAndDerive, DERIVATION_VERSION } from './connect/derive.js';
...
export async function runConnect(opts = {}) {
    ...
    const network = opts.network ?? env.DMEMO_NETWORK ?? 'testnet';
    ...
    const result = await runConnectServer({
        ...
        async onComplete({ txHash, skipped }) {
            if (!derived)
                throw new Error('internal: completion before key derivation');
            ...
            if (txHash) {
                ...
                try {
                    ...
                    if (!receipt)
                        log('  funding tx not confirmed within 120s — it may still land.');
                    else if (receipt.status === 0)
                        log('  funding tx reverted — fund the account manually (see below).');
                    else
                        log('  funding confirmed ✓');
                }
                catch (err) {
            ...
            if (!opts.skipHosts)
                hosts = installDetectedHosts(env, (line) => log(`  ${line}`));
        },
    });
    if (!derived)
        throw new Error('connect finished without deriving an account');
    const account = derived;
    log('');
