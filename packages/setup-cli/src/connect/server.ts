...
function readJson(req: http.IncomingMessage, limitBytes = 64 * 1024): Promise<Record<string, unknown>> {
...
export async function runConnectServer(opts: ConnectServerOptions): Promise<ConnectServerResult> {
  ...
  return await new Promise<ConnectServerResult>((resolve, reject) => {
    let settled = false;
    ...
    server.listen(opts.port ?? 0, '127.0.0.1', () => {
      ...
      timer = setTimeout(() => {
        finish(() =>
          reject(new Error(`timed out after ${Math.round(timeoutMs / 60_000)} minutes waiting for the browser`))
        );
      }, timeoutMs);
      timer.unref?.();
